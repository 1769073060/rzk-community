...
public class MinioUtils {
     ...
     */
    public List<MinIoInfo> listObjects(String bucketName) {
        Iterable<Result<Item>> results = minioClient.listObjects(
                ListObjectsArgs.builder().bucket(bucketName).build());
        List<MinIoInfo> objectItems = new ArrayList<>();
        try {
            for (Result<Item> result : results) {
                Item item = result.get();
                MinIoInfo minIoInfo = new MinIoInfo();
                minIoInfo.setFileName(item.objectName());
                minIoInfo.setFileSize(item.size());
                objectItems.add(minIoInfo);
            }
        } catch (Exception e) {
            e.printStackTrace();
            return null;
